...
<form action="Controller" name="formSearch" method="get">
	<div class="col-sm-2 w-so">
		<strong><fmt:message key="label.sortby" /></strong> <select
			name="sort" class="form-select"
			onchange="document.formSearch.submit();">
			<option value="surname"
				<c:if test="${sort == 'surname' || sort == null}"> selected </c:if>><fmt:message
					key="option.surname" /></option>
			<option value="rating"
				<c:if test="${sort == 'rating'}"> selected </c:if>><fmt:message
					key="option.rating" /></option>
		</select>
	</div>
	<input type="hidden" name="service_id" value="${service_id}"> <input
		type="hidden" name="command"
		value="${service_id == null? 'show_master_list' : 'show_masters_of_service' }">
</form>
	...
	<c:if test="${service == null}">
		<table class="table table-striped">
			<tr>
				...
				<th><fmt:message key="label.short_info" /></th>
				<th><fmt:message key="label.rating" /></th>
				<c:if test="${sessionScope.user != null}">
					<th><fmt:message key="label.view_schedule" /></th>
				</c:if>
			</tr>
			<c:set var="i" value="0"></c:set>
			<c:forEach items="${userlist}" var="item">
				<tr>
					<td><a href="Controller?command=show_user_info&id=${item.id}"><c:out
								value="${item.name} ${item.surname}" /></a></td>
	...
	<c:if test="${service != null}">
		...
		<table class="table table-striped">
			...
			<c:forEach items="${masters}" var="entry">
				<tr>
					<td><a
						href="Controller?command=show_user_info&id=${entry.key.id}"><c:out
								value="${entry.key.name} ${entry.key.surname}" /></a></td>
					<td><c:out value="${entry.key.tel}" /></td>
					<td><c:out value="${entry.key.info}" /></td>
					<td align="center"><c:if test="${entry.key.rating != 0}">
							<c:set var="i" value="${i + 1}"></c:set>
							<h:createstars rating="${entry.key.rating}"
								inputname="${'rat'}${i}" />
					...
					<c:if test="${sessionScope.user != null}">
						<td><a
							href="Controller?command=show_master_schedule&id=${entry.key.id}&date=${LocalDate.now().plusDays(1)}">
								<fmt:message key="label.view_schedule" />
						</a></td>
					</c:if>
				</tr>
			</c:forEach>
