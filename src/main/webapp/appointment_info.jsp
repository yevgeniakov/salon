...
<form action="change_appointment.jsp" method="get">
	...
	<c:if
		...
		<input type="submit" name="submit"
			value=<fmt:message key="button.change_time"/>>
	</c:if>
...
<c:if test="${sessionScope.user.id == appointment.master.id}">
	<form action="Controller" method="post">
		...
		<input type="submit" name="submit"
			value="${appointment.isDone? bincomplete : bcomplete}">
	</form>
	<br>
	<br>
</c:if>
...
<c:if test="${sessionScope.user.role == 'ADMIN'}">
	<form action="Controller" method="post">
		...
		<input type="submit" name="submit"
			value="${appointment.isPaid? bunpaid : bpaid}">
	</form>
	<br>
	<br>
</c:if>
...
<c:if
	...
	<form action="leave_feedback.jsp" method="get">
		...
		<input type="submit" name="submit"
			value="${appointment.rating == 0 ? bleavef : bchangef }">
	</form>
</c:if>
<c:if
	test="${sessionScope.user.role == 'ADMIN' && !(appointment.isDone)}">
	<form action="Controller" method="post" data-confirm=<fmt:message key="confirmation.delete_appointment" />>
		<h:appkeyinputs />
		<input type="hidden" name=command value="delete_appointment" /> <input
