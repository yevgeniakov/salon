...
<div class="appointment-info">
		...
		<div class="card-body">
			<form action="change_appointment.jsp" method="get">
				...
				<c:if
					...
					<br>
					<button class="w-70 btn btn-primary btn-sm text-nowrap"
						type="submit">
	...
	<c:if test="${sessionScope.user.id == appointment.master.id}">
		<div class="card text-center" style="width: 18rem;">
			<div class="card-body">
				<strong><fmt:message key="label.master" /> </strong> <a
					href="my_info.jsp">${appointment.master.name}
	...
	<c:if test="${sessionScope.user.id != appointment.master.id}">
		<div class="card text-center" style="width: 18rem;">
			...
			<div class="card-body">
				<strong><fmt:message key="label.master" /> </strong> <a
					href="Controller?command=show_user_info&id=${appointment.master.id}">${appointment.master.name}
					${appointment.master.surname}</a>
			</div>
		</div>
	</c:if>
	<c:if test="${sessionScope.user.id == appointment.user.id}">
		<div class="card text-center" style="width: 18rem;">
			<div class="card-body">
				<strong><fmt:message key="label.client" /></strong> <a
					href="my_info.jsp">${appointment.user.name}
	...
	<c:if test="${sessionScope.user.id != appointment.user.id}">
		<div class="card text-center" style="width: 18rem;">
			<div class="card-body">
				<strong><fmt:message key="label.client" /></strong> <a
					href="Controller?command=show_user_info&id=${appointment.user.id}">${appointment.user.name}
					${appointment.user.surname} </a>
			</div>
		</div>
	</c:if>
	...
	<div class="card text-center" style="width: 18rem;">
		...
		<br>
		<c:if test="${sessionScope.user.id == appointment.master.id}">
			<fmt:message key="confirmation.complete_appointment"
				var="conf_complete" />
			<fmt:message key="confirmation.incomplete_appointment"
				var="conf_incomplete" />
			<form action="Controller" method="post"
				data-confirm="${appointment.isDone? conf_incomplete : conf_complete}">
				<h:appkeyinputs />
				...
				<button
					class="${appointment.isDone? 'btn btn-danger' : 'btn btn-success'} btn-sm text-nowrap"
					type="submit" name="submit">${appointment.isDone? bincomplete : bcomplete}</button>
			</form>
		</c:if>
	</div>
	...
	<div class="card text-center" style="width: 18rem;">
		...
		<c:if test="${sessionScope.user.role == 'ADMIN'}">
			...
			<form action="Controller" method="post"
				data-confirm="${appointment.isPaid? conf_unpaid : conf_paid}">
				<h:appkeyinputs />
				...
				<button
					class="${appointment.isPaid? 'btn btn-danger' : 'btn btn-success'} btn-sm text-nowrap"
					type="submit" name="submit">${appointment.isPaid? bunpaid : bpaid}</button>
			</form>
		</c:if>
	</div>
	...
	<c:if
		...
		<form action="leave_feedback.jsp" method="get">
			...
			<button
				class="${appointment.rating == 0 ? 'btn btn-primary' : 'btn btn-warning'} text-nowrap"
				type="submit" name="submit">${appointment.rating == 0 ? bleavef : bchangef}</button>
		</form>
	</c:if>
	<c:if
		test="${sessionScope.user.role == 'ADMIN' && !(appointment.isDone)}">
		...
		<form action="Controller" method="post"
			...
			<button type="submit" class="btn btn-danger text-nowrap"
				name="submit">
				<fmt:message key="button.delete_appointment" />
			</button>
		</form>
	</c:if>
</div>
