...
<div class="user-info">
	<h3>
		<fmt:message key="label.user_info" />
		- ${showuser.name} ${showuser.surname}
	</h3>
	<h4>${showuser.role}</h4>
	<c:if test="${showuser.role == 'HAIRDRESSER'}">
		...
		<c:if test="${showuser.rating !=0}">
			<br>
			<h:createstars rating="${showuser.rating}"
				inputname="${'rat'}${showuser.id}" />
			<br>
		</c:if>
	</c:if>
	<c:if
		...
		<br>
		<a
			href="Controller?command=show_appointments_list&user_id=${showuser.id}"><fmt:message
				key="link.user_appointments" /></a>
	</c:if>
	<c:if
		...
		<a
			...
			<fmt:message key="link.view_schedule" />
		</a>
	</c:if>
	<br>
	<form action="Controller" method=post id="profile_form">
		...
		<c:if
			...
			<br>
			<span class="border border-info">${showuser.info}</span>
			<br>
		</c:if>
		<c:if
			test="${showuser.role == 'HAIRDRESSER' && sessionScope.user.role == 'ADMIN'}">
			<textarea class="form-control" rows="10" cols="60" name="info">${showuser.info}</textarea>
			<br>
		</c:if>
		<c:if test="${showuser.role == 'ADMIN'}">
			...
			<input class="form-check-input" type="radio" name="role"
				value="CLIENT"
				<c:if test="${showuser.role == 'CLIENT'}">checked</c:if> disabled />
			...
			<input class="form-check-input" type="radio" name="role"
				value="ADMIN"
				<c:if test="${showuser.role == 'ADMIN'}">checked</c:if> disabled />
	...
	<c:if test="${sessionScope.user.role == 'ADMIN'}">
		...
		<form action="Controller" method=post
			data-confirm="${showuser.isBlocked? conf_unblock : conf_block }">
			<input type="hidden" name="command" value="set_user_block"> <input
				...
				type="hidden" name="id" value="${showuser.id}">
			<fmt:message key="button.block_user" var="block" />
			<fmt:message key="button.unblock_user" var="unblock" />
			<button type="submit"
				class="${showuser.isBlocked? 'btn btn-success' : 'btn btn-danger'}">${showuser.isBlocked? unblock : block}</button>
		</form>
	</c:if>
			...
			<c:forEach items="${services}" var="entry">
				<tr>
					...
					<c:if test="${sessionScope.user.role == 'ADMIN'}">
						<td align="center">
							<form action="Controller" method="post"
								data-confirm=<fmt:message key="confirmation.delete_service"/>>
								<input type="hidden" name=service_id value="${entry.key.id}" />
								<input type="hidden" name=master_id value="${showuser.id}" /> <input
									type="hidden" name=command value="delete_service_from_master" />
								<button type="submit" class="btn btn-danger text-nowrap"
									name="submit">X</button>
							</form>
						</td>
					</c:if>
				</tr>
